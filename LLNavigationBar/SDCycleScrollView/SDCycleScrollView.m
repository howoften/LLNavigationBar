...
- (void)layoutSubviews {
    ...
    if (CGSizeEqualToSize(_dotSize, CGSizeZero) ||
        _spacingBetweenDots < 0.01) {
        return;
    }
    
    CGFloat dotW = _dotSize.width;
    
    //计算圆点间距
    CGFloat marginX = dotW + _spacingBetweenDots;
    
    //计算整个pageControll的宽度
    CGFloat newW = self.subviews.count * dotW + (self.subviews.count - 1) * _spacingBetweenDots;
...
{
    [self scrollToIndex:targetIndex animation:YES];
}
...
- (void)scrollToIndex:(int)targetIndex animation:(BOOL)animation {
    if (targetIndex >= _totalItemsCount) {
        if (self.infiniteLoop) {
            ...
            [_mainView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0] atScrollPosition:[self scrollPosition] animated:NO];
        }
        return;
    }
    [_mainView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0] atScrollPosition:[self scrollPosition] animated:animation];
}
...
- (void)layoutSubviews
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        ...
        if (self.spacingBetweenDots > 0) {
            size = CGSizeMake(self.imagePathsGroup.count * self.pageControlDotSize.width + (self.imagePathsGroup.count - 1) * self.spacingBetweenDots, self.pageControlDotSize.height);
        } else {
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        ...
        if (self.pageControl.superview && !CGRectEqualToRect(pageControlFrame, self.pageControl.frame)) {
//            [self removeAllConstraintsFor:self.pageControl];
            self.pageControl.translatesAutoresizingMaskIntoConstraints = NO;
            [[self.pageControl.topAnchor constraintEqualToAnchor:self.pageControl.superview.topAnchor constant:pageControlFrame.origin.y] setActive:YES];
            ...
            if (_pageControlAliment == SDCycleScrollViewPageContolAlimentRight) {
                [[self.pageControl.rightAnchor constraintEqualToAnchor:self.pageControl.superview.rightAnchor constant:-(CGRectGetWidth(self.pageControl.superview.frame) - CGRectGetMaxX(pageControlFrame))] setActive:YES];
            } else {
                [[self.pageControl.centerXAnchor constraintEqualToAnchor:self.pageControl.superview.centerXAnchor] setActive:YES];
            }
        }else {
            self.pageControl.frame = pageControlFrame;
        }
    ...
    self.layoutBlock != nil ? self.layoutBlock(_mainView, _pageControl) : NULL;
}
...
- (UICollectionViewScrollPosition)scrollPosition {
    if (((UICollectionViewFlowLayout *)_mainView.collectionViewLayout).scrollDirection == UICollectionViewScrollDirectionHorizontal) {
...
- (void)makeScrollViewScrollToIndex:(NSInteger)index{
    [self makeScrollViewScrollToIndex:index animation:YES];
}
...
- (void)removeAllConstraintsFor:(UIView *)view
{
